...
class Courier:
    ...
    Instance Attributes:
        ...
        - location: the current location of the courier as (latitude, longitude)
        - order: the order assigned to the courier (initially none)

    Representation Invariants:
        - self.name != ''
        - -90 <= self.location[0] <= 90
        - -180 <= self.location[1] <= 180
        - not (self.order != None) or self.order.courier is self

    Sample Usage:

    >>> mike = Courier('Mike', (43.671, -79.413))
    >>> mike.order is None
    True
    """
    name: str
