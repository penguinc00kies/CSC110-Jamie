...
class FoodDeliverySystem:
    ...
    def complete_order(self, order: Order, timestamp: datetime.datetime) -> None:
        ...
        """
        order.end_time = timestamp  # Indicate that the order is now "complete"
        order.courier = None  # Make the courier available

    def get_customers(self) -> list[Customer]:
        """Return a list of all customers in this system."""
        return [self._customers[name] for name in self._customers]

    def get_restaurants(self) -> list[Restaurant]:
        """Return a list of all customers in this system."""
        return [self._restaurants[name] for name in self._restaurants]
